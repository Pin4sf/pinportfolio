...
void main() {
    ...
    if (uLowTier) {
        // Single texture read — no chromatic aberration
        vec2 refract = clamp(vUv + offset * uPerturbance, 0.0, 1.0);
        color = texture2D(uBackground, refract).rgb;

        // Single specular highlight only
        vec2 lightDir1 = normalize(vec2(-0.6, 1.0));
        float spec1 = pow(max(0.0, dot(offset, lightDir1)), 4.0);
        color += vec3(0.8, 1.0, 0.85) * spec1 * 0.9;
    } else {
        ...
        color += vec3(0.8, 1.0, 0.85) * spec1 * 0.9;
        color += vec3(1.0, 0.9, 0.7) * spec2 * 0.35;
        color += vec3(0.7, 0.95, 0.8) * fresnel * 0.12;
    }
